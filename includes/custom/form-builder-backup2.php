...
class Contract_Builder {
    ...
    public function __construct() {
        ...
        // AJAX: Only needed for Saving now. "Get HTML" is removed.
    ...
    public function enqueue_assets() {
        ...
        // Pre-generate User Options HTML to pass to JS
        ...
        wp_localize_script( 'cb-script', 'cbParams', array(
            ...
            'opts_student' => $student_opts, // Pass HTML strings to JS
            'opts_teacher' => $teacher_opts
        ));
    }

    // Changed to return HTML string for JS injection
    private function get_user_options_html( $role ) {
        ...
        foreach ( $users as $user ) {
            $html .= sprintf( '<option value="%d">%s (%s)</option>', 
                $user->ID, $user->display_name, $user->user_email 
            );
        }
        return $html;
    }

    // Used for Initial Server Side Render (SSR)
        ...
        <div class="cb-field-wrapper" id="wrapper-<?php echo $id; ?>" data-id="<?php echo $id; ?>">
            ...
            <div class="cb-field-body">
                ...
                <?php elseif ( $type === 'user_select' ): ?>
                    <?php 
                        // Simplified for SSR
                        $role = isset($field['role_limit']) ? $field['role_limit'] : 'student';
                        $u_opts = $this->get_user_options_html($role);
                        // Hacky select injection for SSR
                        $u_opts = str_replace('value="'.$value.'"', 'value="'.$value.'" selected', $u_opts);
                    ?>
                    <select name="<?php echo $name; ?>" class="cb-input-view" <?php echo $required; ?>>
                        <?php echo $u_opts; ?>
                    </select>
                ...
                <?php elseif ( $type === 'image' ): ?>
                    ...
                    <?php if(!empty($value)): ?>
                        <div class="cb-img-preview">
                            ...
                            <!-- Hidden input to preserve value during JS updates if not changed -->
                            <input type="hidden" class="cb-val-store" value="<?php echo esc_attr($value); ?>">
                        </div>
                    <?php endif; ?>
    ...
    public function render_builder( $atts ) {
        ...
        <style>
            #cb-app-root { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; color: #334155; background-color: #f8fafc; padding: 20px; border-radius: 8px; box-sizing: border-box; }
            ...
            #cb-app-root .cb-field-wrapper { position: relative; background: #fff; border: 1px solid transparent; padding: 15px; border-radius: 6px; margin-bottom: 15px; transition: all 0.2s; }
            #cb-app-root .cb-field-wrapper:hover { border-color: #cbd5e1; box-shadow: 0 0 0 4px #f1f5f9; }
            ...
            #cb-app-root .cb-action-btn { background: transparent; border: none; cursor: pointer; color: #94a3b8; font-size: 14px; padding: 4px 8px; }
            #cb-app-root .cb-action-btn:hover { color: #2563eb; }
            #cb-app-root .cb-btn-del:hover { color: #ef4444; }
            #cb-app-root .cb-input-view, #cb-app-root textarea.cb-input-view, #cb-app-root select.cb-input-view { width: 100%; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 6px; font-size: 14px; color: #334155; background: #fff; margin: 0; }
            ...
            #cb-app-root .cb-tool-btn { display: flex; align-items: center; gap: 8px; width: 100%; text-align: left; background: #fff; border: 1px solid #e2e8f0; padding: 10px; border-radius: 6px; font-size: 13px; color: #475569; cursor: pointer; transition: all 0.2s; }
            ...
            #cb-app-root .cb-input-control { width: 100%; padding: 8px 10px; border: 1px solid #cbd5e1; border-radius: 5px; font-size: 13px; color: #1e293b; }
            #cb-app-root .cb-code-font { font-family: monospace; color: #d63384; background: #fff5f9; border-color: #fce7f3; }
            #cb-app-root .cb-badge-btn { font-size: 10px; text-transform: uppercase; font-weight: 700; background: #e2e8f0; color: #64748b; border: none; padding: 2px 8px; border-radius: 4px; cursor: pointer; letter-spacing: 0.5px; }
        ...
        jQuery(document).ready(function($) {
            ...
            // Helper: Safe HTML escaping for attribute values
            ...
            // Core JS Renderer: Reproduces PHP logic
            function renderFieldHTML(field) {
                var icons = {
                    'text': 'fa-font', 'textarea': 'fa-align-left', 'number': 'fa-hashtag',
                    'email': 'fa-envelope', 'date': 'fa-calendar', 'select': 'fa-list-ul',
                    'checkbox': 'fa-check-square', 'user_select': 'fa-user', 'image': 'fa-image'
                };
                ...
                if(field.type === 'textarea') {
                    inputHtml = `<textarea name="${field.name}" class="cb-input-view" placeholder="${escAttr(field.placeholder)}" ${reqAttr}>${field.value}</textarea>`;
                } 
                else if(field.type === 'select') {
                    var optionsHtml = '';
                    if(field.options) {
                        var parts = field.options.split(',');
                        parts.forEach(function(opt){
                            opt = opt.trim();
                            var sel = (field.value === opt) ? 'selected' : '';
                            optionsHtml += `<option value="${escAttr(opt)}" ${sel}>${opt}</option>`;
                        });
                    }
                    inputHtml = `<select name="${field.name}" class="cb-input-view" ${reqAttr}>${optionsHtml}</select>`;
                }
                else if(field.type === 'user_select') {
                    // Inject pre-loaded options from PHP
                    var opts = (field.role_limit === 'teacher') ? cbParams.opts_teacher : cbParams.opts_student;
                    // Basic string select injection (simulating PHP logic)
                    if(field.value) {
                        opts = opts.replace('value="'+field.value+'"', 'value="'+field.value+'" selected');
                    }
                    inputHtml = `<select name="${field.name}" class="cb-input-view" ${reqAttr}>${opts}</select>`;
                }
                else if(field.type === 'checkbox') {
                    var chk = (field.value === '1') ? 'checked' : '';
                    var desc = field.placeholder ? field.placeholder : 'Yes, I agree';
                    inputHtml = `
                     <label class="cb-checkbox-label">
                        <input type="checkbox" name="${field.name}" value="1" ${chk} ${reqAttr}>
                        <span class="cb-desc">${desc}</span>
                     </label>`;
                }
                else if(field.type === 'image') {
                    inputHtml = `<input type="file" name="${field.name}" class="cb-input-file" accept="image/*" ${reqAttr}>`;
                    // Attempt to preserve preview if it exists in DOM or we know value
                    // Note: We can't render the <img> tag easily purely from JS without the URL
                    // But we can check if there's an existing preview wrapper to preserve
                    var existingPreview = $('#wrapper-'+field.id).find('.cb-img-preview');
                    if(existingPreview.length > 0) {
                        inputHtml += '<div class="cb-img-preview">' + existingPreview.html() + '</div>';
                    }
                }
                else {
                    // Default Input
                    inputHtml = `<input type="${field.type}" name="${field.name}" class="cb-input-view" value="${escAttr(field.value)}" placeholder="${escAttr(field.placeholder)}" ${reqAttr}>`;
                }
                ...
                <div class="cb-field-wrapper" id="wrapper-${field.id}" data-id="${field.id}">
                    ...
                    <div class="cb-field-body">
                        ${inputHtml}
                        <div class="cb-meta-tag">Key: <span>${field.name}</span></div>
                    </div>
                </div>`;
            }
            ...
            function updateEditorUI(type) {
                ...
                if (type === 'select') {
                    ...
                    $('#edit-placeholder-label').text('Checkbox Text');
                    $('#edit-options-wrapper').hide();
            ...
            // 1. Add Field (Client Side Render)
            $('.cb-tool-btn').on('click', function(e) {
                ...
                // GENERATE HTML IN JS
                var html = renderFieldHTML(newField);
                
                $('#cb-canvas-area').append(html);
                formState.push(newField);
                $('html, body').animate({ scrollTop: $("#cb-canvas-area").height() }, 300);
            });
            ...
            $('#cb-sync-btn').on('click', function() {
                ...
                if(isNameSynced) $('#edit-name').val( slugify($('#edit-label').val()) );
                updateSyncUI();
            });

            $('#edit-label').on('input', function() {
                if(isNameSynced) $('#edit-name').val( slugify($(this).val()) );
            });
            ...
            // 7. Save Field (Client Side Render)
            $('#cb-save-field').on('click', function() {
                ...
                if (field) {
                    ...
                    // RE-RENDER HTML IN JS
                    var newHtml = renderFieldHTML(field);
